...
- (void)AddAddress:(NSString *)addressStr{
    ...
    CGFloat height;
    if (kScreen_widht == 320) {
        height = 370;
    }
    if (kScreen_widht == 414) {
        height =  538;
    }
    if (kScreen_widht == 375) {
        height = 469;
    }
    
    UIView *backgroundView = [[UIView alloc] initWithFrame:CGRectMake(0, height-40,kScreen_widht , 40)];
    backgroundView.backgroundColor = [UIColor blackColor];
    backgroundView.alpha = 0.6;
    [self addSubview:backgroundView];
    NSLog(@"self.frame.size.height%f", kFit(469));
    UILabel *addressLabel  = [[UILabel alloc] initWithFrame:CGRectMake(20, height-40,kScreen_widht-40 , 40)];
    ...
    [self.descLabelArr addObject:addressLabel];
}
- (void)alterDescLabel {
    ...
    if (!self.curDescLabel) {
        ...
        descLabel.textColor = self.lineColor;
        descLabel.font = [UIFont systemFontOfSize:self.lineWidth *5];
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    NSLog(@"self.drawImageView.backgroundColor %@",self.drawImageView.backgroundColor );
    // 开始绘图
    [self drawImage];
}
...
- (void)drawImage {
    ...
    self.style.lineWidth = self.lineWidth;
    self.style.lineColor = self.lineColor;
